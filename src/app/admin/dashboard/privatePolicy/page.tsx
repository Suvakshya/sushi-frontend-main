// // // components/DashboardComponents/PrivatePolicyManagement.jsx
// // "use client";
...
// // export default function PrivatePolicyManagement() {
...
// //       setLoading(true);
// //       setError("");
// //       const res = await axios.get(`${API}/privatepolicy/policy`);
// //       if (res.data.success) {
// //         setPolicy(res.data.data.description || "");
// //       }
// //     } catch (err) {
// //       console.error("Error fetching policy:", err);
// //       setError("Failed to load private policy");
// //     } finally {
// //       setLoading(false);
// //     }
// //   };
...
// //       if (res.data.success) {
// //         setIsEditing(false);
// //         fetchPolicy(); // Refresh to get latest
// //       }
// //     } catch (err) {
// //       console.error("Error saving policy:", err);
// //       setError(err.response?.data?.message || "Failed to save private policy");
// //     } finally {
// //       setSaving(false);
// //     }
// //   };
...
// //   const cancelEdit = () => {
// //     setIsEditing(false);
// //     fetchPolicy(); // Reset to original content
// //   };
...
// //           placeholder="Enter private policy content..."
// //         />
// //       ) : (
// //         <div className="prose max-w-none p-4 border border-gray-200 rounded-lg bg-gray-50">
// //           {policy ? (
// //             <div className="whitespace-pre-line">{policy}</div>
// //           ) : (
// //             <p className="text-gray-500 italic">No private policy set yet.</p>
// //           )}
// //         </div>
// //       )}

// //       <div className="mt-4 text-sm text-gray-600">
// //         <p>Character count: {policy.length}</p>
// //         <p>Last updated: {policy ? new Date().toLocaleDateString() : 'Never'}</p>
// //       </div>
// //     </div>
// //   );
// // }
...
//           <p className="mt-4 text-gray-600">Loading private policy...</p>
//         </div>
//       </div>
//     );
//   }
...
//           <div className="flex justify-between items-center mb-6">
//             <h2 className="text-2xl font-semibold text-gray-800">
//               <span className="text-[#EF5350]">Private</span> Policy
//             </h2>
...
//             <p>Character count: <span className="font-semibold text-[#EF5350]">{policy.length}</span></p>
//             <p>Last updated: {policy ? new Date().toLocaleDateString() : 'Never'}</p>
//           </div>
//         </div>
//       </div>
...
export default function PrivatePolicyManagement() {
  ...
  const savePolicy = async () => {
    try {
      ...
      if (res.data.success) {
        setIsEditing(false);
        fetchPolicy(); // Refresh content
      }
    } catch (err) {
      console.error('Error saving policy:', err);
      const errorMessage =
        (err as AxiosError<{ message?: string }>)?.response?.data?.message ||
        'Failed to save private policy';
      ...
      <div className="max-w-4xl mx-auto">
        <div className="bg-white rounded-xl shadow-lg p-6 border border-[#EF5350]/40">
          ...
          <div className="mt-4 text-sm text-gray-600 bg-[#FFEBEE] p-3 rounded-lg border border-[#EF5350]/40">
            ...
            <p>
              Last updated:{' '}
              {policy ? new Date().toLocaleDateString() : 'Never'}
            </p>
          </div>
        </div>
      </div>
